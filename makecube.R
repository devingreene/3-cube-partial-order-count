require("diagram")
for(f in grep("^mat",dir(),value=TRUE))
{
	s=sub("mat","mats",f)%in%dir();
	A <- matrix(scan(f),ncol=8,nrow=8,byrow=FALSE,
		...
		pos <-
		c(1, 3, 3, 1)
		file <- paste(sub("mat","cube",f),".ps",sep="") 
		...
		plotmat(A[name,name], pos, curve = 0, name,
				box.size = if(s) 0.08 else 0.07,segment.from=0.1,
				endhead=T,shadow.size = 0,box.type=if(s)
				"diamond" else "circle",
				arr.pos = A.[name,name],
